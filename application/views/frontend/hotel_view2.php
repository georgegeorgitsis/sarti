<div class="container">
    <?php if (isset($hotel) && !empty($hotel)): ?>   
        ...
        <?php if(isset($hotel_images) && count($hotel_images) > 0): ?>
        <div class="row">
            <div class="col-md-12 clearfix no-padding">
                <ul id="imageGallery">
                    
                    <?php foreach($hotel_images as $h_i_key => $h_i):?>
                        <li data-thumb="<?= base_url('assets/uploads/' . $h_i['image_name']) ?>" 
        ...
        <?php if(isset($hotel_rooms) && $hotel_rooms):?>
            <div class="row mt flex flex-center flex-wrap">
                <?php foreach ($hotel_rooms as $room): ?>
                    <?php if(isset($room['has_prices']) && $room['has_prices']): ?>
                        <div class="col-md-6 room-prices">
                            <h4 class="text-center text-semibold"> <?= $room['room_type_name'] ?> - <?= $room['floor'] ?> </h4>
                            <table class="table table-responsive room-prices-table datatables">                                
                                <thead>
